# !/usr/bin/env python
# -- coding: utf-8 --
# @Time : 2023/11/23 15:38
# @Author : liumin
# @File : EigenCAM_for_YOLO8.py


pretrain_weight = '/home/liumin/python_code/ultralytics/runs/detect/stringing_glue_cell/weights/best.pt'
...
class yolov8_heatmap:
    def __init__(self, weight, cfg, device, method, layer, backward_type, conf_threshold, ratio):
        ...
        model = Model(cfg, ch=1, nc=len(model_names)).to(device)
        print(model)
    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            img_cp = np.squeeze(img, 0).copy()
            img_cp = cv2.cvtColor(img_cp, cv2.COLOR_GRAY2BGR)
            cam_image = show_cam_on_image(img_cp, saliency_map, use_rgb=True)
...
def get_params():
    params = {
        'weight': 'runs/detect/stringing_glue_cell/weights/best.pt',
...
if __name__ == '__main__':
    ...
    model(r'20231026164508A.bmp', 'result')